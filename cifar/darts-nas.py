...
from darts.model import NetworkCIFARTest as Network
import pandas as pd
from utils.net_utils import test
from utils.attacks_cifar import apgd_attack_cifar

parser = argparse.ArgumentParser(description='Adversarial attacks on CIFAR(DARTS)')
...
print("Average acc of AutoPGD-10 on DARTS: {}".format(apgd))    
# if args.to_csv:
#     try:
...
#         print("Run {} Average acc on {} starts of PGD on {}: {}".format(run, n_repeats, 'darts', pgd_accs[n_repeats]))

#         apgd = apgd_attack_cifar(net, testloader, args.eps, args.iters, n_repeats)
#         apgd_accs[n_repeats] = apgd
#         print("Run {} Average acc on {} starts of AutoPGD on {}: {}".format(run, n_repeats, 'darts', apgd))    
...
# df.to_csv('darts_cifar10_pgd.csv', index=False)
